...
| [`maw-rs`](maw-rs/) | `maw serve` + maw-ui | Browser UI for [maw](https://github.com/Soul-Brews-Studio/maw-rs), a tmux fleet orchestrator for AI coding agents. Ingress-only; ships claude, codex and omx so `maw wake` has something to launch. ⚠️ ~1.04 GB, mostly those CLIs. amd64 only (upstream publishes no aarch64 Linux binary). |
...
| [`thclaws-chat`](thclaws-chat/) | Built-in thCLAWS `chatbot` GUI shell | Focused ingress-only chat sidebar backed by the already-running `thclaws` add-on. No second agent process, copied credentials, or published port; the original thCLAWS app remains unchanged. |
